...
const postgresSha256PasswordHash = function (password, random64code, token, server_iteration, isSM3) {
  if (typeof password !== 'string') {
    throw new Error('RFC5802: client password must be a string')
  }
...
const postgresMd5Sha256PasswordHash = function (password, random64code, md5Salt) {
  ...
  // Although RFC5802 is using "Server Key" to generate the serverKey,
  // Both OpenGauss jdbc driver & py-opengauss are using "Sever Key"
  // Just as "pgjdbc/src/main/java/org/postgresql/util/MD5Digest.java"
  // According to openGauss-server->"src/common/backend/libpq/sha2.cpp", it is using "Sever Key", which is different from RFC5802 definition
  // So we are following the same convention here.
